...
<img src="chrome/icons/icon128.png" alt="TimeShift Logo" width="96" height="96" />
...
[![Version](https://img.shields.io/badge/Version-1.0.1-6366f1?style=for-the-badge)](https://github.com/sheikhabirali/timeshift/releases)
...
│   ├── privacy-policy.html     # Privacy policy (zero data collection)
│   └── icons/                  # Extension icons (16, 32, 48, 128px)
│
├── firefox/                    # Firefox extension (Manifest V2)
│   ├── manifest.json           # MV2 manifest with gecko settings
│   ├── background.js           # Background script — tabs.executeScript
│   ├── content.js              # Content script (browser API compat)
│   ├── content.css             # Overlay styles (shared)
│   ├── popup.html              # Popup (shared)
│   ├── popup.js                # Popup logic (browser API compat)
│   ├── timezones.js            # Timezone database (shared)
...
# Done! The extension is now active in your browser.
```
...
# Done! Extension runs until Firefox is closed.
# For permanent install, use the signed .xpi from Firefox Add-ons.
```
...
### Chrome (Manifest V3)
- **Service Worker** (`background.js`) — registers the context menu and handles `chrome.scripting.executeScript()` for on-demand content script injection
- **Content Script** (`content.js`) — injected only when the user triggers conversion via right-click. Idempotent (guarded by `window.__timeshiftLoaded`)
- **Popup** (`popup.html` + `popup.js`) — standalone manual conversion interface with timezone search
- **Storage** — `chrome.storage.sync` for cross-device preferences (from/to timezone settings)
- **Zero host permissions** — uses `activeTab` only, not `<all_urls>` in the content_scripts declaration

### Firefox (Manifest V2)
- **Background Script** — uses `browser.tabs.executeScript()` (no Scripting API in Firefox MV2)
- **Browser Compatibility Shim** — `const tsApi = typeof browser !== 'undefined' ? browser : chrome` across all files
- **`data_collection_permissions`** — declared in `gecko` settings as empty arrays (collects nothing)
- All core logic is shared with the Chrome version

### Timezone Engine
- Custom `convertTime()` using `Intl.DateTimeFormat` and offset calculation — handles DST automatically
...
- Day shift detection (+1 / 0 / -1) with exact date shown
...
Have an idea? [Open a Feature Request →](https://github.com/sheikhabirali/timeshift/issues/new?template=feature_request.md)
...
- Firefox version published with MV2 + browser API compat
...
- 🐛 **Reporting bugs** via [GitHub Issues](https://github.com/sheikhabirali/timeshift/issues)
